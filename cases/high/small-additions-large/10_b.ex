...
def final?(%Workflow{state: state}) do
  @transitions |> Map.get(state, []) |> Enum.empty?()
end
...
def visited(%Workflow{} = flow) do
  flow |> history() |> Enum.map(fn record -> record.to end) |> Enum.uniq()
end

def steps(%Workflow{history: history}) do
  length(history)
end

def reset(%Workflow{} = flow) do
  {:ok, %Workflow{flow | state: :draft, history: []}}
end
